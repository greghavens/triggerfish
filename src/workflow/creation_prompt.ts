...
5. **Conditions & branching** — "Are there any conditional paths? Should certain steps only run in specific situations?"
6. **Self-healing** — "Should this workflow have an autonomous healing agent? It watches execution in real time, triages failures, spawns specialist teams to diagnose and fix issues, and proposes workflow updates for your approval."

Do NOT ask about error handling or retries — the workflow engine handles that automatically.
Do NOT use todo tools during the interview — plan mode is sufficient for tracking progress.
